...
class Qwen2VLImageProcessor(BaseImageProcessor):
    ...
    def visualize_uigraph(
        self, patch_assign, image_size, patch_size, image, uimask_ratio
    ):
        """
        Visualize UI Graph with uniform patch dropping.
        ...
        Args:
            ...
            image: Input image.
            uimask_ratio: Proportion of patches to be dropped per component (default: 0).
        ...
        for comp_id in unique_components:
            ...
            if (
                len(y_indices) >= 2 and uimask_ratio > 0
            ):  # Only drop if there are more than 2 patches
                num_to_drop = round(len(y_indices) * uimask_ratio)
                if num_to_drop > 0:
                    # Random_drop
        ...
        # blurred = cv2.GaussianBlur(image, (15, 15), 0)  # Adjust kernel size if needed
