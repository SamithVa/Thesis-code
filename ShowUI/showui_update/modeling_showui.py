...
class ShowUICausalLMOutputWithPast(ModelOutput):
    ...
    # visual_tokens: Optional[torch.LongTensor] = None
...
class Qwen2VLFlashAttention2(Qwen2VLAttention):
    ...
    def forward(
        ...
        # )
        # cos, sin = self.rotary_emb(value_states, seq_len=rotary_seq_len) # TODO Problem with Rotary Embedding
        ...
        if past_key_value is not None:
            # print('use cache', cos.shape, sin.shape)
            cache_kwargs = {
                "sin": sin,
                "cos": cos,
                "cache_position": cache_position,
        ...
        **kwargs,
        ...
        if patch_pos is not None and layer_skip_ratio != 0:
            ...
            if use_cache: # for decoding only
                hidden_states[:, retain_mask] = block_outputs[0] # only update those selected hidden_state output
...
class ShowUIForConditionalGeneration(ShowUIPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.visual = ShowUIVisionTransformerPretrainedModel._from_config(
    ...
    @replace_return_docstrings(
        output_type=ShowUICausalLMOutputWithPast, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        input_ids: torch.LongTensor = None,
