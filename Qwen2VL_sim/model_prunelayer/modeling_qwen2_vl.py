...
#         self.hidden_size = config.hidden_size
#         self.layer_idx = layer_idx
#         self.retain_ratio = getattr(config.vision_config, "retain_ratio", 1)
...
#             # print(retain_mask.shape, retain_mask.sum())
...
#                 hidden_states[:, retain_mask] = block_outputs[0]
...
class Qwen2VLVisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        merge = self.merger(hidden_states)
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    def __init__(self, config: Qwen2VLConfig):
        ...
        self.prune_layer = getattr(config, "prune_layer", None)
        self.print_tflops = getattr(config, "print_tflops", False)
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if i == self.prune_layer and select_mask is not None: # select_mask will be None after first token generation
                    retain_mask = select_mask[0] 
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if (
                ...
                )  # replace the image_emb to inputs_embed
                select_mask[(input_ids == self.config.image_token_id)] = visual_select_mask
                # print(select_mask.shape, select_mask.sum())
