...
class ShowUIProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        # Screenshot -> Graph
        self.uigraph_train = kwargs.get(
            "uigraph_train", True
        )  # Enable ui graph during training
        self.uigraph_test = kwargs.get(
            "uigraph_test", False
        )  # Enable ui graph during inference
        ...
        # Graph -> Mask
        self.uimask_pre = kwargs.get(
            "uimask_pre", False
        )  # Prebuild patch selection mask in the preprocessor (not in model layers)
    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2VLProcessorKwargs],
        ...
        if images is not None:
            ...
            output_kwargs['images_kwargs']['max_pixels'] = 256 * 28 * 28

            # lower resolution
            image_inputs_low = self.image_processor(
                images=images,
                videos=None,
                uigraph_use=uigraph_use,
                uigraph_diff=self.uigraph_diff,
        ...
        for i in range(len(text_inputs["input_ids"][0])):
            ...
            if (
                text_inputs["input_ids"][0, i] == 151652
            ):  # <|vision_start|> in Qwen2VL vocabulary
                cur_img_len = (
                    image_inputs["image_grid_thw"][cur_img_idx].prod() // merge_length
                )
                text_inputs["patch_pos"][0, i + 1 : i + 1 + cur_img_len] = image_inputs[
                    "patch_assign"
