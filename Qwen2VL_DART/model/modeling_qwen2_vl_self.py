...
from .src.modeling_qwen2_vl import (
...
class DART(Qwen2VLModel):
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        cache_position: Optional[torch.LongTensor] = None,
        image_token_num: Optional[torch.LongTensor] = None,
    ) -> Union[Tuple, BaseModelOutputWithPast]:
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training: # training 
            ...
            else: # inference, DART is only work as a plug and play
                DART_config = self.config.DART_config
                if DART_config is not None:
                    if image_token_num != None:
                        self.config.DART_config['image_token_length'] = image_token_num 
                        DART_config['image_token_length'] = image_token_num 
                ...
                # print(f'{i} - layer_output', layer_outputs[0].shape)
            # print(f'{i} - hidden state', hidden_states.shape)
            hidden_states = layer_outputs[0] # layer_output : [hidden_state, key_value_cache]
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # start_visual_idx = torch.where(input_ids == self.config.vision_start_token_id)[0]
                image_token_num = image_mask.sum() 
                update_image_token_num = True
        ...
        outputs = self.model(
            ...
            image_token_num = image_token_num if update_image_token_num else None
        )
