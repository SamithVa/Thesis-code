...
parser.add_argument(
    "--imgs_annot", type=str, required=False, help="json file for reference test data"
)
parser.add_argument(
    "--pred_path", type=str, required=True, help="json file for predicted test data"
)
parser.add_argument(
    "--output_dir", type=str, required=True, help="output directory for images"
)
...
def action2step(action, image_size, return_bbox=False):
    ...
    if action_type in ["CLICK", "HOVER", "ENTER"]:
        ...
        action_step = '{{"action_type": {}, "click_point": {}, "value": "{}"}}'.format(
            2, click_point, select_value
        )
    elif action_type == "TYPE":
        typed_text = action["operation"]["value"]
        action_step = '{{"action_type": {}, "click_point": {}, "value": "{}"}}'.format(
            3, click_point, typed_text
        )
...
mind2web_test_annot = args.imgs_annot

if mind2web_imgs_dir == None:
    mind2web_imgs_dir = (
        "/data/data1/syc/intern/wanshan/mind2map_dataset/mind2web_images"
    )

mind2web_test = json.load(
    open(
        (
            "/data/data1/syc/intern/wanshan/mind2map_dataset/mind2web_data_test_"
            + task
            + ".json"
            if mind2web_test_annot == None
            else mind2web_test_annot
        ),
        "r",
    )
)
...
if pred_path == None:
    pred_path = (
        f"/home/syc/intern/wanshan/SeeClick/agent_tasks/mind2map_{task}_result.json"
    )
...
print(
    f"Operation mismatched count: {op_false_count}/{len(merge_df)} \nElement mismatched count: {ele_false_count}/{len(merge_df)}"
)
...
def save_images_with_annotations(
    imgs_list, bboxes, acts, ref_acts, instructions, output_dir, annot_ids
):
    """
    ...
    for i, img_path in tqdm(
        enumerate(imgs_list), total=len(imgs_list), desc="Saving images"
    ):
        ...
        ax.add_patch(
            patches.Rectangle(
                (0, img_height), img_width, text_bg_height, color="white", lw=0
            )
        )
        ...
        plt.savefig(
            os.path.join(output_dir, f"{annot_ids[i]}_{count}.png"),
            bbox_inches="tight",
            pad_inches=0,
        )
        plt.close(fig)
...
for index, key in enumerate(mismatch_types):
    ...
    save_images_with_annotations(
        imgs_list, bboxes, acts, ref_acts, instructions, imgs_output_dir, annot_ids
    )
